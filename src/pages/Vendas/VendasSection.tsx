  ...
  if (!venda) {
      return (
          <div className="p-4 space-y-4 text-center">
              <h1 className="text-3xl font-bold text-foreground">Venda não encontrada</h1>
              <p className="text-muted-foreground">A venda com o ID "{id}" não foi localizado no sistema.</p>
            ...
            <header className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        {isEditing ? "Modo de edição ativado" : "Informações completas da venda"}
                    </p>
                </div>
                <div className="flex gap-2">
                    {isEditing ? (
                        ...
                        <>
                            ...
                            <Button variant="outline" onClick={() => navigate('/vendas')}><ArrowLeft className="h-4 w-4 mr-2" /> Voltar</Button>
                        </>
                    )}
                </div>
            </header>
                ...
                <CardContent className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            ...
                            {renderField("Nome do Colaborador", "nomeColaborador", venda.nomeColaborador)}
                            {renderField("Nome do Supervisor", "nomeSupervisor", venda.nomeSupervisor)}
                            {renderField("Nome do Cliente", "nomeCliente", venda.cliente.nome)}
                            {renderField("Cpf do Cliente", "cpfCliente", venda.cliente.cpf)}
                        </div>
                    </div>
                    <div className="space-y-4">
                        <h3 className="text-lg font-semibold text-foreground border-b pb-2">Informaçoes</h3>
                        <div className="space-y-3">
                            {renderField("Banco", "banco", venda.banco)}
                            {renderField("Agente", "agente", venda.agente)}
                            {renderField("Linha de Operacao", "linhaOperacao", venda.linhaOperacao)}
                            {renderField("Cidade da Venda", "cidadeVenda", venda.cidadeVenda)}
                            {renderField("Promotora", "promotora", venda.promotora)}
                        </div>
                    </div>
                    <div className="space-y-4">
                        <h3 className="text-lg font-semibold text-foreground border-b pb-2">Dados Financeiros</h3>
                        <div className="space-y-3">
                            {renderField("Valor Venda", "valorLiberado", (venda.valorLiberado), "valorLiberado")}
                            {renderField("Prazo", "prazo", (venda.prazo), "prazo")}
                            {renderField("Taxa", "taxa", (venda.taxa), "taxa")}
                            {renderField("Data de Pagamento", "dataPagamento", (venda.dataPagamento), "date")}
                            {renderField("Comissao do Colaborador", "comissaoColaborador", (venda.comissaoColaborador), "comissaoColaborador")}
                        </div>
                    </div>
                </CardContent>
            </Card>
